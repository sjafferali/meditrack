...
@router.post("/medications/{medication_id}/dose", response_model=DoseInDB, status_code=status.HTTP_201_CREATED)
def record_dose(
    medication_id: int,
    ...
    doses_today = db.query(Dose).filter(
        and_(
...
def get_doses(
    ...
    doses = db.query(Dose).filter(
        Dose.medication_id == medication_id
    ).order_by(Dose.taken_at.desc()).offset(skip).limit(limit).all()
