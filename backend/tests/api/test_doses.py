...
class TestDoses:
    ...
    def test_get_doses_by_date(self, client, sample_medication, db_session):
        ...
        response = client.get(
            f"/api/v1/doses/medications/{sample_medication.id}/doses/2023-01-15"
        )
        assert response.status_code == 200
    ...
    @pytest.mark.unit
    def test_record_dose_for_date_max_reached(
    ...
    def test_get_daily_summary_by_date(self, client, sample_medication, db_session):
        ...
        med_summary = next(
            m for m in data["medications"] if m["medication_id"] == sample_medication.id
        )
        assert med_summary["doses_taken"] == 1
