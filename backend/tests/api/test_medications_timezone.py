...
def test_medication_dose_count_with_timezone(client: TestClient, db_session: Session):
    ...
    dose_time_pst = datetime.now(user_tz).replace(hour=23, minute=0, second=0, microsecond=0)
    dose1 = Dose(
        medication_id=medication.id,
        taken_at=dose_time_pst
    )
    ...
    response = client.get(
        "/api/v1/medications/",
        params={"timezone_offset": user_timezone_offset}
    )
    assert response.status_code == 200
    data = response.json()
...
def test_record_dose_and_get_medications_timezone_consistency(client: TestClient, db_session: Session):
    """Test that recording doses and getting medications use consistent timezone logic"""
    # Create a medication
    medication = Medication(
        name="Test Med",
        dosage="10mg",
