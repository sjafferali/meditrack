...
class TestMedicationsWithDate:
    ...
    @pytest.mark.unit
    def test_get_medications_without_date_uses_today(self, client, sample_medication, db_session):
    ...
    @pytest.mark.unit
    def test_get_single_medication_with_date(self, client, sample_medication, db_session):
        ...
        response = client.get(f"/api/v1/medications/{sample_medication.id}?date=2023-01-15")
        assert response.status_code == 200
    ...
    @pytest.mark.unit
    def test_date_filtering_respects_boundaries(self, client, sample_medication, db_session):
    ...
    def test_date_consistency_across_endpoints(self, client, sample_medication, db_session):
        ...
        response = client.get(f"/api/v1/medications/{sample_medication.id}?date={test_date}")
        single_med_data = response.json()
        
        # Check doses endpoint for the date
        response = client.get(f"/api/v1/doses/medications/{sample_medication.id}/doses/{test_date}")
        ...
        med_summary = next(
            m for m in summary_data["medications"] 
